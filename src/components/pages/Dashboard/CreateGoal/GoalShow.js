...
const GoalShow = () => {
    ...
    const { data: goalData, isLoading, refetch } = useQuery('goalData', () => fetch(`https://top-gear-perform-server.up.railway.app/usergoal/${user?.email}`, {
        method: 'GET',
    })
        .then(res => res.json()))
    refetch()
    ...
    const handleDelete = (id) => {
        const url = `https://top-gear-perform-server.up.railway.app/goal/${id}`
        fetch(url, {
            method: 'DELETE',
        })
            .then(res => res.json())
