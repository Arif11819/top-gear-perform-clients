...
const DayOffModal = ({ vacations, setOpenModal }) => {
    ...
    useEffect(() => {
        fetch(`https://top-gear-perform-server.vercel.app/namevacation?type=${dayOffType}&email=${user?.email}`)
            .then(res => res.json())
    ...
    const handleSubmit = () => {
        ...
        const url = `https://top-gear-perform-server.vercel.app/vacation/${dayOffType}`
        fetch(url, {
            method: 'PUT',
            headers: {
        ...
        fetch('https://top-gear-perform-server.vercel.app/vacationstore', {
            method: 'POST',
            headers: {
                'content-type': 'application/json'
            },
