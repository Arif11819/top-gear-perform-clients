...
const ScheduleData = ({ schedule }) => {
    ...
    const { data: scheduleTask, isLoading, refetch } = useQuery('scheduleTask', () => fetch(`https://top-gear-perform-server.up.railway.app/schedule/${user?.email}`, {
        method: 'GET',
    })
        .then(res => res.json()))
    refetch()
    ...
    const handleDeleteTask = id => {
        ...
        const url = `https://top-gear-perform-server.up.railway.app/schedule/${id}`
        fetch(url, {
            method: 'DELETE'
        })
            .then(res => res.json())
